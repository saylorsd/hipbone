...
def query_property_sales(d3_ids):
    ids_sql_list = ', '.join([str(d3_id) for d3_id in d3_ids])
    query = f"SELECT sale_date, sale_price, grantor, grantee, sale_terms, verified_by, sale_instrument FROM property_sales WHERE d3_id IN ({ids_sql_list}) ORDER BY sale_date DESC"
    return execute(query)
