...
from .queries import query_db, query_blight_violations, query_building_permits, query_demolitions, query_voters, \
    query_ownership, query_parcel_tax_and_values, query_property_sales, aggregate_voters, query_d3_table
                            ...
                            if 'parcel_id' in json_data:
                                ...
                                if 'regions' in json_data and 'us_census_tract' in json_data['regions']:
                                    if 'name' in json_data['regions']['us_census_tract']:
                                        self.parcel_data['Census tract'] = json_data['regions']['us_census_tract'][
                                            'name']
                                        parcel['parcel_data'] = {
                                            'Census tract': json_data['regions']['us_census_tract']['name']}
...
def get_parcels(request):
    ...
    if not request.user.is_authenticated:
        return JsonResponse(
            {'error_message': "User is not logged in and therefore unable to get this data. Try the /login/ URL.",
             'reload': True})
    ...
    blight_violations_config = {'table_name': "blight_violations",
                                'name_by_field': OrderedDict([("ticket_number", "Ticket Number"),
                                                              ...
                                                              ("violation_code_and_description", "Violation"),
                                                              # Combine violation_code, violation_description like this f"{violation_code}: {violation_description}"
    ...
    building_permits_config = {'table_name': "building_permits",
                               'name_by_field': OrderedDict([("permit_no", "Permit Number"),
                                                             ...
                                                             ("owner_name", "Owner"),
                                                             # owner_first_name, owner_last_name ==> Display as (FIRST if exists) (LAST)
                                                             ("owner_address", "Owner Address"),
                                                             # [ ] Some comments on some of these building permit fields still need to be copied over from the specs.
    ...
    if PRODUCTION:
        ...
        ownership = [
            {'d3_year': 2019, 'owner_name': 'Jetson,George', 'owner_address': "019409013 Space Way, Satellite B87ZZ9"},
    ...
    parcel_tax_and_values_grouped = group_by_record(parcel_tax_and_values,
                                                    parcel_tax_and_values_config['name_by_field'])
